...
- **Results** (`?results=0` to hide; on by default in season) is the second overlay: `useResults`
  ...
  score on the Sheet and the ○●休 strip on the List, `Bouts` lists the day's card, selected by who
  is fighting (cross-division bouts are published on the Makuuchi card only). An absence counts
